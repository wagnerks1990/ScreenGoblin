...
- Non-owner account creation is an acknowledged offline, create-only command,
  never an HTTP route. It may create exactly one membership with role `ADMIN`,
  `PUBLISHER`, or `VIEWER` and the exact compatibility grants, but never an
  `OWNER`. Non-exact existing normalized emails fail closed; an exact rerun may
  only verify an unchanged, still-unexpired original result. The user,
  membership, grants, fixed database-time 24-hour bootstrap marker, and tenant
  audit events commit atomically; no rerun can reset or extend the credential.
  The temporary password must never be logged and permits only forced rotation
