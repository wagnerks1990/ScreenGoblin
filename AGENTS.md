...
`docs/UX_REFERENCE.md` records the approved ScreenGoblin layout and workflow direction for the Operations Console. Use only ScreenGoblin-specific reference renderings for this repository; never use Herd Store, LabGoblin, RoomGoblin, or unrelated project imagery as implementation reference. Renderings are conceptual UX references and must not be treated as evidence that a feature is implemented.
