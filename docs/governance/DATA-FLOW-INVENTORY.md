...
| ------------------ | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Console browser    | Staff credentials, content metadata, operational state   | Access token plus bounded unresolved release-command bodies and idempotency keys in per-tab `sessionStorage`; other UI state in memory | TLS API requests                                                  | Script-readable bearer storage increases XSS impact; unresolved commands are retained only for exact safe retry after an ambiguous result. Production SSO/MFA, HttpOnly browser sessions, and scoped authorization are not implemented                                                                                                                     |
| Control-plane API  | Auth, publishing metadata, enrollment, heartbeat         | PostgreSQL records and audit events                                                                                                    | Database, Redis, private object storage, signed manifests         | P-256 proof, revocation, staged targeted initial enrollment/re-enrollment, ordinary release-candidate approval, and assignment-bound private media delivery exist; upload/quarantine/scanning, scoped grants, MFA/re-authentication, attestation, automatic overlapping rotation, verified erasure, and complete transactional audit coverage remain gates |
...
| Player             | Pairing/re-enrollment material, manifests, media         | Keystore identity, public credential metadata, active and last-known-good media                                                        | Pairing candidates, proof challenges, heartbeat, manifest polling | Native proof with staged targeted initial enrollment and explicit fresh-key re-enrollment is implemented; attestation, offline recall, automatic overlapping rotation, verified erasure, and physical-device validation remain gates                                                                                                                       |
...
3. **Publish and assignment:** author → API immutable organization-wide
   candidate → author submit → distinct owner/admin approval → authorized
   publisher → transactional schedule, immutable assignment, publication
   provenance, and audit. The publisher may equal the author or approver, but
   the approver cannot equal the author. Direct schedule publication fails
   closed. The Console operates create, exact-evidence review, submit, approve,
   publish, and withdrawal. Scoped grants, MFA/re-authentication,
   rejection/revision, rollback, and complete operational evidence remain
   target-state work; emergency stays disabled.
