...
| Control-plane API  | Auth, publishing metadata, enrollment, heartbeat         | PostgreSQL records and audit events                                             | Database, Redis, object storage, signed manifests                 | P-256 proof, revocation, staged targeted re-enrollment, and bounded binary media metadata exist; upload scanning/private delivery, scoped approval, attestation, automatic overlapping rotation, verified erasure, and complete transactional audit coverage remain gates |
...
2. **Content preparation:** approved external process → allowlisted immutable
   object origin → API metadata registration. The Console inventory is read-only;
   browser upload and web media are disabled. Registration and publication
   enforce type, size, hash, expiry, and aggregate release limits, but no
   production-safe quarantine, scanning, transcoding, private delivery, or
   object lifecycle pipeline exists.
