...
| Control-plane API  | Auth, publishing metadata, enrollment, heartbeat         | PostgreSQL records and audit events               | Database, Redis, object storage, signed manifests | Upload security, scoped approval, complete transactional audit coverage, and server-side device proof of possession remain gates |
