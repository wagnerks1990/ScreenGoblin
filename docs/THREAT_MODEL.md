...
| Synchronized or stalled heartbeat clients       | Fleet load spike, false offline status, or exhausted client work      | A newly activated credential sends once immediately, while ordinary process starts and reconnect/resume catch-up are phase-spread. The Player then uses one recursive single-flight timeout. Strict 5-second-to-one-day server cadence validation, bounded cadence jitter of up to plus or minus 10%, pull-forward-only coalesced wakeups, and bounded retryable backoff with an uncapped `Retry-After` floor reduce synchronized load without bypassing cooldown. Non-retryable protocol failures retain the current cadence, and every attempt creates fresh telemetry and one-use proof instead of replaying a request. Representative Android suspend/resume and network-transition evidence remains an open pre-production gate.                                                                                                                                                                                                                   |
...
| Android packaged-surface drift                  | Release candidate exposes an unintended Android surface               | CI parses the manifest from the assembled release APK rather than trusting only the source manifest. A fail-closed exact allowlist binds package and SDK values; INTERNET and boot permissions; optional TV/touch features; explicit backup and cleartext denial; absent debug, test-only, legacy-storage, network-security override, shared-user, queries, instrumentation, and library declarations; the exported launcher intent surface; and the enabled non-exported boot receiver. Aliases, services, and providers are forbidden, and the retained JSON summary records both APK and packaged-manifest SHA-256 values. This static check does not establish APK signing/provenance, dependency or runtime safety, OWASP MASVS compliance, or behavior on physical devices.                                                                                                                                                                       |
...
| Local audit row mutation                        | Incomplete or misleading local history                                | Bounded scalar/JSON fields and a PostgreSQL trigger reject ordinary row updates and direct deletes while preserving current user and tenant deletion semantics. The API uses a separate non-owning runtime login denied direct audit update/delete, trigger changes, and truncation. It can still append fabricated rows and change most domain rows across tenants because RLS is absent; the migration owner and PostgreSQL/platform administrators can also bypass the local boundary. There is no hash chain, independently protected anchor, outbox/export monitor, approved retention job, or legal hold, so local audit is not trustworthy under database-credential compromise.                                                                                                                                                                                                                                                                 |
...
deletion. The non-owning API runtime role is denied direct mutation, while the
separately held migration owner and PostgreSQL/platform administrators remain
...
The Compose deployment separates the PostgreSQL schema/migration owner from a
non-owning runtime login. Migrations run with the owner, then an idempotent
reconciliation removes runtime ownership, inherited roles, schema/DDL,
trigger, truncation, migration-ledger, and direct append-only-history mutation
authority before the API, seed, or recovery job starts. It specifically prevents
that login from schema/trigger changes, truncation, migration-ledger access,
existing audit/tombstone rewrite or deletion, and organization/user deletion.
It does not provide tenant isolation at the database layer: the runtime role
retains broad cross-tenant CRUD on most application tables and can append audit
rows. It also does not constrain the migration credential, PostgreSQL
superusers, hosting-provider administrators, backups, or storage operators. A
managed database that cannot apply the checked role attributes,
PUBLIC/default-privilege revocations, and object grants must supply and validate
an equivalent provider-specific boundary before deployment.
