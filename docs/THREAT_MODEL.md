...
| Android packaged-surface drift                  | Release candidate exposes an unintended Android surface               | CI validates the assembled APK's packaged manifest, not only its source. An exact allowlist binds package/SDK; INTERNET and boot access; the package-derived signature-level `DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION` declaration and use; optional TV/touch features; safe application flags; the exported launcher; the non-exported boot receiver; and one non-exported Startup provider with only ProcessLifecycle and EmojiCompat initializers. Merge rules remove the ProfileInstaller initializer/receiver and DUMP permission. JSON evidence records analyzer version plus APK/manifest hashes. This static check is not APK signing/provenance, dependency or runtime safety, OWASP MASVS compliance, or physical-device evidence.                                                                                                                                                                                                           |
